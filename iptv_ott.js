...
// version: 1.0.0
// description: Простой IPTV плеер для Lampa с поддержкой M3U и EPG
...
(function () {
    ...
    function IPTVPlugin() {
        ...
        this.init = function () {
            console.log('IPTV Plugin: Init start');
            ...
            Lampa.Settings.add({
                ...
                onContext: function () {
                    Lampa.Select.show({
                        title: 'IPTV Lite',
                        items: [
                            {title: 'Открыть плеер', action: 'open'},
                            {title: 'Настроить ссылки', action: 'settings'}
                        ],
                        onSelect: function (item) {
                            if (item.action === 'open') Lampa.Activity.push({url: '', title: 'IPTV Lite', component: 'iptv_lite', page: 1});
                            else Lampa.Settings.main('iptv_lite_settings');
                        }
                    });
                }
            });
        ...
        this.render = function (object) {
            var m3u_url = storage.get('iptv_m3u_link', '');
            var epg_url = storage.get('iptv_epg_link', '');
            ...
            this.create = function () {
                var _this = this;
                if (!m3u_url) {
                    items_container.append('<div class="empty">Укажите ссылку на M3U в настройках</div>');
            ...
            this.parse = function (str) {
                ...
                lines.forEach(function (line) {
                    ...
                    if (line.indexOf('#EXTINF') === 0) {
                        ...
                        currentItem.name = nameMatch ? nameMatch[1] : 'Без названия';
            ...
            this.showGroups = function () {
                ...
                Object.keys(groups).forEach(function (gName) {
                    var item = Lampa.Template.get('button_category', {title: gName});
            ...
            this.showChannels = function (gName) {
                ...
                groups[gName].forEach(function (chan) {
                    ...
                    item.on('hover:enter', function () {
                        ...
                        // Передаем данные для EPG в плеер
                        Lampa.Player.play(video);
                        Lampa.Player.playlist([video]);
                        
                        if (epg_url) {
                            // Инъекция EPG id если Lampa поддерживает через доп. параметры
                            video.tvg_id = chan.epg_id;
                            video.xmltv = epg_url;
                        }
                    });
                    items_container.append(item);
